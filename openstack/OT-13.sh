...
if [[ "$found_file" == false ]]; then
    echo "[OT-13] N/A: No files to check."
		if [ -e "openstack_report.csv" ]; then
			echo "파일 권한 관리,$CODE,네트워킹 서비스 설정파일 소유권 설정,상,N/A" >> openstack_report.csv
			echo "[OT-13] Report generated."
		else
...
if [ "$VULN" = true ]; then
    REPORT="파일 권한 관리,$CODE,네트워킹 서비스 설정파일 소유권 설정,상,취약"
else
    REPORT="파일 권한 관리,$CODE,네트워킹 서비스 설정파일 소유권 설정,상,양호"
fi
