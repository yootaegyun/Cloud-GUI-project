...
# CSV 파일 경로 설정
report_csv_path = py_path+'/../vulscan_csv/openstack_report.csv'
solution_csv_path = py_path+'/openstack_solution.csv'
result_xlsx_path = py_path+'/../output/openstack_report.xlsx'
result_pdf_path = py_path+'/../output/openstack_report.pdf'
...
with pd.ExcelWriter(result_xlsx_path) as writer:
    ...
    diagnosis_df.to_excel(writer, sheet_name='OpenStack 진단결과')
...
with PdfPages(result_pdf_path) as pdf:
    ...
    column_widths = [0.20, 0.09, 0.53, 0.09, 0.09]
    ...
    # 표 제목
    title_text = 'OpenStack 진단결과 (페이지 1)'
    ax.text(0.5, 0.05, title_text, fontsize=18, fontweight='bold', ha='center', va='center') #0.5 x 좌표 0.05 y좌표(표와의거리)
    
    pdf.savefig()
    plt.close()

    # OpenStack 진단결과 시트 (페이지 2)
    if len(diagnosis_df) > 20:
        fig, ax = plt.subplots(figsize=(8.27, 11.69))  # A4 규격(210mm x 297mm)
        ax.axis('off')
       
        table = ax.table(cellText=diagnosis_df.values[20:40], colLabels=diagnosis_df.columns, cellLoc='center')
        table.auto_set_font_size(False)
        table.set_fontsize(8)  # 표 내용 글꼴 크기 조정
        table.scale(1, 2.5)  # 표 크기 조정
        ...
        # 표 제목
        title_text = 'OpenStack 진단결과 (페이지 2)'
        ax.text(0.5, 0.05, title_text, fontsize=18, fontweight='bold', ha='center', va='center') #0.5 x 좌표 0.05 y좌표(표와의거리)
        
        pdf.savefig()
        plt.close()
                
    # OpenStack 진단결과 시트 (페이지 3)
    if len(diagnosis_df) > 40 :
        fig, ax = plt.subplots(figsize=(8.27, 11.69))  # A4 규격(210mm x 297mm)
        ax.axis('off')
        
        table = ax.table(cellText=diagnosis_df.values[40:], colLabels=diagnosis_df.columns, cellLoc='center' )
        table.auto_set_font_size(False)
        table.set_fontsize(8)  # 표 내용 글꼴 크기 조정
        table.scale(1, 2.5)  # 표 크기 조정
        ...
        # 표 제목
        title_text = 'OpenStack 진단결과 (페이지 3)'
        ax.text(0.5, 0.05, title_text, fontsize=18, fontweight='bold', ha='center', va='center') #0.5 x 좌표 0.05 y좌표(표와의거리)
    ...
    # 표 제목
    title_text = 'OpenStack 조치사항 (페이지 1)'
    ...
    if len(actions_df) > 10 and len(table_data) >= 10:
        fig, ax = plt.subplots(figsize=(8.27, 11.69))  # A4 규격(210mm x 297mm)
        ax.axis('off')
        ax.set_title('OpenStack 조치사항 (페이지 2)', fontsize=18)  # 제목 글꼴 크기 조정
        ...
        # 표 제목
        title_text = 'OpenStack 조치사항 (페이지 2)'
        ax.text(0.5, 0.05, title_text, fontsize=18, fontweight='bold', ha='center', va='center')
    ...
    if len(actions_df) > 20 and len(table_data) >= 20:
        fig, ax = plt.subplots(figsize=(8.27, 11.69))  # A4 규격(210mm x 297mm)
        ax.axis('off')
        ax.set_title('OpenStack 조치사항 (페이지 3)', fontsize=18)  # 제목 글꼴 크기 조정
        ...
        # 표 제목
        title_text = 'OpenStack 조치사항 (페이지 3)'
        ax.text(0.5, 0.05, title_text, fontsize=18, fontweight='bold', ha='center', va='center')
    ...
    if len(actions_df) > 30 and len(table_data) >= 30:
        fig, ax = plt.subplots(figsize=(8.27, 11.69))  # A4 규격(210mm x 297mm)
        ax.axis('off')
        ax.set_title('OpenStack 조치사항 (페이지 4)', fontsize=18)  # 제목 글꼴 크기 조정
        ...
        # 표 제목
        title_text = 'OpenStack 조치사항 (페이지 4)'
        ax.text(0.5, 0.05, title_text, fontsize=18, fontweight='bold', ha='center', va='center')
