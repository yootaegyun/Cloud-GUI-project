...
# CSV 파일 경로 설정
report_csv_path = py_path+'/../vulscan_csv/mysql_report.csv'
solution_csv_path = py_path+'/mysql_solution.csv'
result_xlsx_path = py_path+'/../output/mysql_report.xlsx'
result_pdf_path = py_path+'/../output/mysql_report.pdf'
...
with PdfPages(result_pdf_path) as pdf:
    ...
    table.set_fontsize(11)  # 표 내용 글꼴 크기 조정
    ...
    # 표 제목
    title_text = 'My-SQL 진단결과 (페이지 1)'
    ax.text(0.5, 0.05, title_text, fontsize=18,
            fontweight='bold', ha='center', va='center')
    pdf.savefig()
    plt.close()

    # My-SQL 진단결과 시트 (페이지 2)
    if len(diagnosis_df) > 20:
        fig, ax = plt.subplots(figsize=(8.27, 11.69))  # A4 규격(210mm x 297mm)
        ax.axis('off')
        ax.set_title('My-SQL 조치사항 (페이지 2)', fontsize=18)  # 제목 글꼴 크기 조정
        table = ax.table(
            cellText=diagnosis_df.values[20:], colLabels=diagnosis_df.columns, cellLoc='center')
        table.auto_set_font_size(False)
