...
py_path = sys.path[0]
df = pd.read_csv(py_path+'/../vulscan_csv/apache_report.csv')
graph_path = (py_path+'/../output/apache_graph.pdf')
...
# 계정관리 항목의 점검결과 계산
result1 = df[df['구분'] == '보안설정']['점검결과'].value_counts()
result1 = result1.reindex(['양호', '취약', '점검 불가'], fill_value=0)

# 파일 및 디렉토리 관리의 점검결과 계산
result2 = df[df['구분'] == '접근관리']['점검결과'].value_counts()
result2 = result2.reindex(['양호', '취약', '점검 불가'], fill_value=0)

# 서비스 관리 항목의 점검결과 계산
result3 = df[df['구분'] == '패치관리']['점검결과'].value_counts()
