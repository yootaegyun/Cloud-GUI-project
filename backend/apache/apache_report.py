...
# CSV 파일 경로 설정
py_path = sys.path[0]
report_csv_path = py_path+'/../vulscan_csv/apache_report.csv'
solution_csv_path = py_path+'/apache_solution.csv'
result_xlsx_path = py_path+'/../output/apache_report.xlsx'
result_pdf_path = py_path+'/../output/apache_report.pdf'
...
with PdfPages(result_pdf_path) as pdf:
    ...
    if len(diagnosis_df) > 20:
        fig, ax = plt.subplots(figsize=(8.27, 11.69))  # A4 규격(210mm x 297mm)
        ax.axis('off')
        ax.set_title('Apache 조치사항 (페이지 2)', fontsize=18)  # 제목 글꼴 크기 조정
        table = ax.table(
            cellText=diagnosis_df.values[20:], colLabels=diagnosis_df.columns, cellLoc='center')
        table.auto_set_font_size(False)
        table.set_fontsize(12)  # 표 내용 글꼴 크기 조정
