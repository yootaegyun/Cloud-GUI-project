...
df = pd.read_csv(py_path+'/../vulscan_csv/linux_report.csv')
graph_path = py_path+'/../output/linux_graph.pdf'
...
# 원그래프 데이터 계산
result5 = df['점검결과'].value_counts()
result5 = result5.reindex(['양호', '취약'], fill_value=0)
...
with PdfPages(graph_path) as pdf:
    ...
    wedges, _, autotexts = ax[1, 1].pie(
        result5, labels=result5.index, colors=colors, autopct='%1.1f%%', startangle=90)
    ax[1, 1].set_title('Linux 진단결과' )
...
#Remove the empty subplot
    ...
    plt.suptitle('Linux 취약점 진단 점검 결과', fontsize=25, fontweight='bold', x=0.5, y=0.95)
