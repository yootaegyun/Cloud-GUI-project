...
if [ ! -f "$FILEPATH" ]; then
    echo "[$CODE] N/A: $FILEPATH does not exist."
		if [ -e "linux_report.csv" ]; then
			echo "파일 및 디렉토리 관리,$CODE,/etc/(x)inetd.conf 파일 소유자 및 권한 설정,상,N/A" >> linux_report.csv
...
if [ "$VULN" = true ]; then
    REPORT="파일 및 디렉토리 관리,$CODE,/etc/(x)inetd.conf 파일 소유자 및 권한 설정,상,취약"
else
    REPORT="파일 및 디렉토리 관리,$CODE,/etc/(x)inetd.conf 파일 소유자 및 권한 설정,상,양호"
fi
